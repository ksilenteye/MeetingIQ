...
uvicorn main:app --reload --port 8000        # dashboard at http://localhost:8000
...
cd .. && node --test extension/tests/*.test.mjs               # 96 tests, no deps
```
...
- **Switching roots requires corroboration.** `createRootArbiter()` only abandons an *attached* root when a challenger beats it by `SWITCH_MARGIN` (12) on `SWITCH_STREAK` (3) consecutive evaluations — up to ~2.4 s at the 800 ms discovery throttle, during which the old root is still bound and still capturing. A *detached* root rebinds immediately; waiting there would be pure loss.
